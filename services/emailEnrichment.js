...
Format the response as a JSON object with these fields:
{
  ...
  async processEmailWithRetry(email, retryCount = 0, emitCallback = null) {
    try {
      ...
      if (retryCount < EmailEnrichmentService.CONFIG.maxRetries) {
        const delay = EmailEnrichmentService.CONFIG.retryDelay * Math.pow(2, retryCount); // Exponential backoff
        console.log(`⏳ Waiting ${delay/1000} seconds before retry...`);
        await new Promise(resolve => setTimeout(resolve, delay));
        return this.processEmailWithRetry(email, retryCount + 1, emitCallback);
      }
      
      // ALWAYS save error status to database
      await Email.findByIdAndUpdate(email._id, {
        $set: {
          'aiMeta.error': `Failed after ${EmailEnrichmentService.CONFIG.maxRetries} attempts: ${error.message}`,
          'aiMeta.enrichedAt': new Date(),
          'aiMeta.version': '1.0',
          isProcessed: false
        }
      ...
      console.log(`❌ Email ${email.id} failed after ${EmailEnrichmentService.CONFIG.maxRetries} attempts, error saved to database`);
      ...
      try {
        if (emitCallback) {
          emitCallback('mail:enrichmentStatus', {
            messageId: email.id,
            status: 'error',
            message: `Failed after ${EmailEnrichmentService.CONFIG.maxRetries} attempts: ${error.message}`,
            error: true
          });
        }
      } catch (socketError) {
  ...
  async processEnrichmentBatch(emails, emitCallback = null) {
    console.log(`🔄 Processing batch of ${emails.length} emails`);
    
    // Process emails in chunks
    const results = [];
    for (let i = 0; i < emails.length; i += EmailEnrichmentService.CONFIG.batchSize) {
      const chunk = emails.slice(i, i + EmailEnrichmentService.CONFIG.batchSize);
      console.log(`📧 Processing chunk of ${chunk.length} emails`);
      
      const chunkResults = await Promise.allSettled(
        chunk.map(email => this.processEmailWithRetry(email, 0, emitCallback))
      );
      results.push(...chunkResults);
      
      // Add a delay between chunks
      if (i + EmailEnrichmentService.CONFIG.batchSize < emails.length) {
