...
export const initMailSocket = (socket, io) => {
  ...
  socket.on('mail:init', async ({ appUserId, email }) => {
    ...
    if (!user?.outlookSync?.firstSyncComplete) {
      console.log('🆕 First-time sync: prompting for range...');
      return socket.emit('mail:promptDateRange');
    }
  });

  // 🗂 Date Range Sync + AI
  socket.on('mail:syncInRange', async ({ appUserId, email, startDate, endDate }) => {
    const token = await getToken(appUserId, email, 'outlook');
    if (!token) return socket.emit('mail:error', 'Token not found');

    const messages = await fetchEmailsInRange(token, startDate, endDate);

    for (const msg of messages) {
      const exists = await Email.findOne({ id: msg.id });
      if (exists) {
        socket.emit('mail:new', exists);
        continue;
      }
      ...
      // Create email first without AI metadata
      ...
      // Emit the initial email
      socket.emit('mail:new', emailDoc);

      // Start enrichment in background
      try {
        await emailEnrichmentService.enrichEmail(emailDoc, socket);
      } catch (error) {
    ...
    await User.findOneAndUpdate(
      ...
      {
        $set: {
          'outlookSync.firstSyncComplete': true,
          'outlookSync.firstSyncedRange': {
